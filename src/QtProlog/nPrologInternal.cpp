...
#pragma message("PL_atom_to_term missing")
//  if (!PL_atom_to_term(in_atom.lsi, out_term.lsi, out_bindings.lsi)) return false;
...
#pragma message("PL_copy_term missing")
...
Prolog_impl::Term::operator Term::Binding (void) const
{
  ...
  const Term& me = *this;
  if ( functor().NotEqual(equals_functor) || ( me[0].type() != tt_atom ) )
